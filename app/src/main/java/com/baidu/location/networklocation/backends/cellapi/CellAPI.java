package com.baidu.location.networklocation.backends.cellapi;
...
public class CellAPI extends OnlineDataSource implements ILocationSource<CellSpec> {
	...
	public Collection<LocationSpec<CellSpec>> retrieveLocation(Collection<CellSpec> cellSpecs) {
		...
		for (CellSpec spec : cellSpecs) {
			try {
				if (BaiduNetworkLocationService.DEBUG) Log.d(TAG, "retrieveLocation: checking for " + spec);
				...
				if (BaiduNetworkLocationService.DEBUG) Log.wtf(TAG, "retrieveLocation: exception=" + e, e);
			}
		}
		if (BaiduNetworkLocationService.DEBUG) Log.d(TAG, "retrieveLocation: locationSpecs=" + locationSpecs);
		...
		) throws Exception
	{
		...
		map.put("la", la);
		map.put("ci", ci);
		if (BaiduNetworkLocationService.DEBUG) Log.d(TAG, "resolve: parameter=" + map);
		URLConnection conn = new URL(SERVICE_URL + "?" + encode(map)).openConnection();
		...
		if (BaiduNetworkLocationService.DEBUG) Log.d(TAG, "resolve: meta=" + conn.getRequestProperties());
		...
		if (BaiduNetworkLocationService.DEBUG) Log.d(TAG, "resolve: response=" + ret);
		return ret;
	}
}
