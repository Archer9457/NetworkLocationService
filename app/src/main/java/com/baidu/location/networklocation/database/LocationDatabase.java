package com.baidu.location.networklocation.database;
...
public class LocationDatabase {
	...
	private <T extends PropSpec> LocationSpec<T> get(final byte[] identBlob) {
		...
		if (BaiduNetworkLocationService.DEBUG) Log.d(TAG, "retrieved identBlob=" + Arrays.toString(identBlob) + ", locationSpec=" + locationSpec);
		return locationSpec;
	}
	...
	private <T extends PropSpec> void insert(byte[] identBlob, LocationSpec<T> locationSpec) {
		...
		try {
			...
			if (BaiduNetworkLocationService.DEBUG) Log.d(TAG, "inserted identBlob=" + Arrays.toString(identBlob) + ", locationSpec=" + locationSpec);
		} catch (Exception e) {
			Log.w(TAG, e);
		}
		finally {
